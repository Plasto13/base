...
trait Filters
{
    ...
    public function addFilter($options, $values = false, $filter_logic = false, $fallback_logic = false)
    {
        ...
        $filter = new BaseFilter($options, $values, $filter_logic);
    ...
    /**
     * Determine if the current action is a list operation (using standard or ajax DataTables).
     * @return bool
     */
...
class BaseFilter
{
    ...
    public function checkOptionsIntegrity($options)
    {
        ...
        if (! \View::exists('base::filters.'.$options['type'])) {
