  ...
  <div class="container-fluid">
    ...
    <div class="navbar-header">
        <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
          <span class="sr-only">{{ trans('pimodule::settings.table.toggle_filters') }}</span>
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
        </button>
        <a class="navbar-brand" href="#">{{ trans('pimodule::settings.table.filters') }}</a>
    </div>
        ...
        <ul class="nav navbar-nav">
          <!-- THE ACTUAL FILTERS -->
			@foreach ($base['filters'] as $filter)
				@include($filter['view'])
			@endforeach
          <li><a href="#" id="remove_filters_button"><i class="fa fa-eraser"></i> {{ trans('pimodule::settings.table.remove_filters') }}</a></li>
        </ul>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
</nav>
